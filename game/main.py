...
NPC_NAMES = [
    ...
    "David Bowie", "Baby Girl", "David Lynch", "David Byrne"
]
...
class SpaceStationGame:
    ...
    def on_closing(self):
        ...
        if self.player_data and self.player_data.get("Player Name"):
    ...
    def update_battery(self):
        ...
        if "station_power" not in self.character_data:
            self.character_data["station_power"] = {
                "battery_level": 25.0,
                "solar_charging": False,
                "last_update_time": datetime.datetime.now().isoformat(),
                "system_levels": {
        ...
        try:
            ...
            last_update = datetime.datetime.fromisoformat(self.character_data["station_power"]["last_update_time"])
            now = datetime.datetime.now()
            elapsed_seconds = (now - last_update).total_seconds()
            
            # Get system power levels from player data
            system_levels = self.character_data["station_power"]["system_levels"]
            ...
            if self.character_data["station_power"]["solar_charging"]:
            ...
            self.character_data["station_power"]["last_update_time"] = now.isoformat()
            ...
            self.character_data["station_power"]["last_update_time"] = datetime.datetime.now().isoformat()
    ...
    def check_life_support_status(self, elapsed_seconds):
        ...
        try:
            ...
            life_support_level = self.character_data["station_power"]["system_levels"].get("life_support", 10)
            
            # Combine player and NPCs for processing
            all_crew = [self.character_data] + self.station_crew
            ...
            for crew_member in all_crew:
                is_player = (crew_member == self.character_data)
    ...
    def handle_oxygen_death(self):
        ...
        try:
            ...
            # Reset oxygen damage
            self.character_data["damage"]["oxygen"] = 50
            
            # Set all limbs to critical levels
            for limb in self.character_data["limbs"]:
                self.character_data["limbs"][limb] = 20
            ...
            self.character_data["location"] = {"x": 0, "y": 0}
            self.show_room()
        except Exception as e:
    ...
    def trigger_power_outage(self):
        ...
        try:
            ...
            self.character_data["station_power"]["battery_level"] = 0.1
    ...
    def process_pending_trades(self):
        ...
        if "pending_trades" in self.character_data["stock_market"]:
            pending = self.character_data["stock_market"]["pending_trades"]
            if pending and (pending.get('bought') or pending.get('sold')):
                # Check if we already have a trade log for this cycle
                if "trade_log" not in self.character_data["stock_market"]:
                    self.character_data["stock_market"]["trade_log"] = []
                
                # Check if there's already an entry for the current cycle
                current_cycle_log = None
                for entry in self.character_data["stock_market"]["trade_log"]:
                ...
                if current_cycle_log and has_trades:
                    ...
                    self.character_data["stock_market"]["trade_log"].append(trade_entry)
                
                # Clear pending trades
                self.character_data["stock_market"]["pending_trades"] = {}
    ...
    def update_market_data(self):
        ...
        # Update player data
        self.character_data["stock_market"].update({
    ...
    def load_market_data(self):
        ...
        if "stock_market" in self.character_data:
            market_data = self.character_data["stock_market"]
    ...
    def show_main_menu(self):
        ...
        new_game_btn = tk.Button(button_frame, text="Story Mode", font=("Arial", 14), width=15, command=self.start_story_mode)
        new_game_btn.pack(pady=10)
        
        new_game_btn = tk.Button(button_frame, text="Debug Mode", font=("Arial", 14), width=15, command=self.debug_mode)
        new_game_btn.pack(pady=10)
    ...
    def debug_mode(self):
        jobs = ["Staff Assistant", "Engineer", "Security Guard", "Doctor", "Captain", "Bartender", "Head of Personnel",
                "Botanist"]
        self.show_character_creation(jobs)
    ...
    def new_player_screen(self):
        ...
        # Title
        title_label = tk.Label(self.root, text="New Player Screen", font=("Arial", 24), bg="black", fg="white")
        ...
        # Name input
        name_label = tk.Label(form_frame, text="Please Enter a new player:", font=("Arial", 14), bg="black", fg="white")
        name_label.grid(row=0, column=0, sticky="w", pady=10)

        self.player_name_entry = tk.Entry(form_frame, font=("Arial", 14), width=25)
        self.player_name_entry.grid(row=0, column=1, pady=10)

        # Buttons
        button_frame = tk.Frame(self.root, bg="black")
        button_frame.pack(pady=20)

        start_game_btn = tk.Button(button_frame, text="Create Player Profile", font=("Arial", 14), width=15,
    ...
    def create_player(self):
        self.player_data["Player Name"] = self.player_name_entry.get().strip()
        Game.save_game(self.player_data)
        self.story_mode_screen()

    def start_story_mode(self):
        if Game.check_player_saves(self.base_path):
            self.new_player_screen()
        else:
            self.show_load_game()

    def story_mode_screen(self):
        # Unbind mousewheel if it was bound
        if hasattr(self.root, 'mousewheel_bound') and self.root.mousewheel_bound:
            self.root.unbind_all("<MouseWheel>")
            self.root.mousewheel_bound = False
        ...
        # Title
        title_label = tk.Label(self.root, text=f"Story Mode for {self.player_data.get("Player Name")}", font=("Arial", 24), bg="black", fg="white")
        title_label.pack(pady=50)
        ...
        achievement_btn = tk.Button(button_frame, text="Achievements", font=("Arial", 14), width=15,
                                    command=self.achievements_popup)
        achievement_btn.pack(pady=10)

        notes_btn = tk.Button(button_frame, text="Notes", font=("Arial", 14), width=15, command=self.notes_popup)
        notes_btn.pack(pady=10)

        unlocks_btn = tk.Button(button_frame, text="Unlocks", font=("Arial", 14), width=15,
                                command=self.unlocks_popup)
        unlocks_btn.pack(pady=10)
        if self.player_data.get("Round") == 0:
            start_round_btn = tk.Button(button_frame, text="Start Round", font=("Arial", 14), width=15,
                                    command=self.start_round_creation)
            start_round_btn.pack(pady=10)
        else:
            start_round_btn = tk.Button(button_frame, text="Rejoin Round", font=("Arial", 14), width=15,
                                        command=self.rejoin_round)
            start_round_btn.pack(pady=10)
    ...
    def rejoin_round(self):
        self.load_character(self.player_data.get("Player Name"))

    def achievements_popup(self):
        # Show dialog with the room window as parent to keep focus within the room
        self.root.after(10, lambda: messagebox.showinfo("Achievements", "Achievements Coming Soon!",
                                                                 parent=self.root))
        # Make sure the window stays on top after dialog
        self.root.after(20, self.root.lift)
        self.root.focus_force()

    def notes_popup(self):
        # Show dialog with the room window as parent to keep focus within the room
        self.root.after(10, lambda: messagebox.showinfo("Notes", "Notes Coming Soon!",
                                                        parent=self.root))
        # Make sure the window stays on top after dialog
        self.root.after(20, self.root.lift)
        self.root.focus_force()

    def unlocks_popup(self):
        # Show dialog with the room window as parent to keep focus within the room
        self.root.after(10, lambda: messagebox.showinfo("Unlocks", "Unlocks Coming Soon!",
                                                        parent=self.root))
        # Make sure the window stays on top after dialog
        self.root.after(20, self.root.lift)
        self.root.focus_force()

    def start_round_creation(self):
        jobs = ["Staff Assistant"]
        self.show_character_creation(jobs)

    def show_character_creation(self, jobsInput):
        # Clear the window
        for widget in self.root.winfo_children():
            widget.destroy()
    ...
    def start_game(self):
        ...
        self.character_data["job"] = self.job_var.get()
        self.character_data["inventory"] = []
        self.character_data["location"] = {"x": -1, "y": 0}
        self.character_data["stock_holdings"] = {}
        
        # Initialize damage stats
        self.character_data["damage"] = {
            "burn": 0,
            "poison": 0,
            "oxygen": 0
        }
        ...
        if job == "Staff Assistant":
            self.character_data["credits"] = 1000
        elif job == "Engineer":
            self.character_data["credits"] = 2500
        elif job == "Security Guard":
            self.character_data["credits"] = 5000
        elif job == "Doctor":
            self.character_data["credits"] = 7500
        elif job == "Captain":
            self.character_data["credits"] = 10000
        elif job == "Bartender":
            self.character_data["credits"] = 3500
        elif job == "Head of Personnel":
            self.character_data["credits"] = 9000
        elif job == "Botanist":
            self.character_data["credits"] = 3000
        else:
            self.character_data["credits"] = 1000  # Default for Staff Assistant
            
        # Set job-specific permissions for room access
        if job == "Captain":
            # Captain has access to all stations
            self.character_data["permissions"] = {
        ...
        if character_name in available_names:
             available_names.remove(character_name) # Avoid duplicate names
        ...
        if "stock_market" not in self.character_data:
            self.character_data["stock_market"] = {
                "cycle_number": 1,
                "day_number": 1,
                "last_update_time": datetime.datetime.now().isoformat(),
                "companies": [],
            ...
            for company in self.companies:
                self.character_data["stock_market"]["companies"].append({
                    "name": company.name,
                    "current_value": company.current_value,
                    "previous_value": company.previous_value,
                    "price_history": company.price_history,
    ...
    def save_and_start(self):
        ...
        self.update_market_data()
        self.player_data["Character"] = self.character_data
        Game.save_game(self.player_data)
    ...
    def show_room(self):
        ...
        name_label = tk.Label(info_frame, text=f"Name: {self.character_data['name']}", font=("Arial", 12), bg="black", fg="white")
        name_label.pack(side=tk.LEFT, padx=20)
        
        job_label = tk.Label(info_frame, text=f"Job: {self.character_data['job']}", font=("Arial", 12), bg="black", fg="white")
        job_label.pack(side=tk.LEFT, padx=20)
        
        credits_label = tk.Label(info_frame, text=f"Credits: {self.character_data['credits']:.2f}", font=("Arial", 12), bg="black", fg="white")
    ...
    def show_character_sheet(self):
        ...
        # Name and job
        name_label = tk.Label(info_frame, text=f"Name: {self.character_data['name']}", font=("Arial", 14), bg="black", fg="white")
        name_label.pack(anchor="w", padx=10, pady=5)
        
        job_label = tk.Label(info_frame, text=f"Job: {self.character_data['job']}", font=("Arial", 14), bg="black", fg="white")
        job_label.pack(anchor="w", padx=10, pady=5)
        
        # Credits
        credits_label = tk.Label(info_frame, text=f"Credits: {self.character_data['credits']}", font=("Arial", 14), bg="black", fg="white")
        ...
        # Inventory button
        inv_count = len(self.character_data.get('inventory', []))
        ...
        holdings_count = len(self.character_data.get('stock_holdings', {}))
        ...
        # Notes button
        notes_count = len(self.character_data.get('notes', []))
        ...
        for damage_type in damage_types:
            damage_value = self.character_data["damage"].get(damage_type["key"], 0)
        ...
        for i, limb_name in enumerate(limb_order[:3]):
            if limb_name in self.character_data['limbs']:
                health = self.character_data['limbs'][limb_name]
        ...
        limb_health = sum(self.character_data["limbs"].values()) / len(self.character_data["limbs"])
        damage_health = 100 - (sum(self.character_data["damage"].values()) / len(self.character_data["damage"]))
    ...
    def show_notes_popup(self):
        ...
        # Load notes
        if "notes" in self.character_data and self.character_data["notes"]:
            ...
            for note in reversed(self.character_data["notes"]):
                # Format the note
    ...
    def add_note(self, text):
        ...
        if "notes" not in self.character_data:
            self.character_data["notes"] = []
        ...
        self.character_data["notes"].append(note)
    ...
    def show_inventory_popup(self):
        ...
        player_inventory = self.character_data.get('inventory', [])
    ...
    def _get_selected_item_from_inventory(self, inventory_list):
        ...
        current_player_inventory = self.character_data.get("inventory", [])
    ...
    def show_holdings_popup(self):
        ...
        if not self.character_data.get('stock_holdings', {}):
            holdings_list.insert(tk.END, "You don't own any company stocks.")
        else:
            for company, shares in self.character_data['stock_holdings'].items():
    ...
    def save_game_and_close(self, save_window):
        ...
        filename = os.path.join(saves_path, f"{self.character_data['name']}.json")
    ...
    def take_item(self, item_id):
        ...
        player_inventory = self.character_data.setdefault("inventory", [])
    ...
    def open_stock_market(self):
        ...
        market = StockMarket(self.root, self.character_data, self.companies,
    ...
    def update_player_data(self, updated_data):
        ...
        self.character_data = updated_data
    ...
    def use_door(self):
        ...
        x = self.character_data["location"]["x"]
        y = self.character_data["location"]["y"]
    ...
    def show_hallway(self):
        ...
        if location.get("locked", False):
            ...
            self.character_data["location"]["x"] = prev_x
            self.character_data["location"]["y"] = prev_y
        ...
        if "station_power" in self.character_data:
            battery_level = self.character_data["station_power"].get("battery_level", 100.0)
        ...
        # Location info
        coords_label = tk.Label(self.root, text=f"Location: {self.character_data['location']['x']} North, {self.character_data['location']['y']} East",
    ...
    def return_to_hallway(self):
        ...
        if hasattr(self, 'before_character_sheet_location'):
            self.character_data["location"]["x"] = self.before_character_sheet_location["x"]
            self.character_data["location"]["y"] = self.before_character_sheet_location["y"]
    ...
    def move_direction(self, direction):
        ...
        # Update location
        self.character_data["location"]["x"] = x
        self.character_data["location"]["y"] = y
        ...
        if loc_key not in self.ship_map:
            ...
            if x == 6 and y == 5 and self.ship_map.get("6,6", {}).get("name") == "Security":
                ...
                self.character_data["location"]["x"] = 6
                self.character_data["location"]["y"] = 6
            # Special case for Engineering Bay entrance
                ...
                self.character_data["location"]["x"] = x - (1 if direction == "north" else -1 if direction == "south" else 0)
                self.character_data["location"]["y"] = y - (1 if direction == "east" else -1 if direction == "west" else 0)
    ...
    def damage_random_limb(self, min_damage, max_damage):
        ...
        # Get a random limb
        limb = random.choice(list(self.character_data["limbs"].keys()))
    ...
    def damage_limb(self, limb, min_damage, max_damage):
        ...
        if limb in self.character_data["limbs"]:
            # Calculate damage
            damage = random.randint(min_damage, max_damage)
            
            # Apply damage
            original_health = self.character_data["limbs"][limb]
            self.character_data["limbs"][limb] = max(0, original_health - damage)
            
            # Format the limb name for display
            limb_name = limb.replace('_', ' ').title()
            
            # Show damage message
            messagebox.showinfo("Blunt Injury", f"Your {limb_name} took {damage}% blunt damage and is now at {self.character_data['limbs'][limb]}%.")
            
            # Add note about the injury
            self.add_note(f"Suffered blunt damage to {limb_name}: Took {damage}% damage (from {original_health}% to {self.character_data['limbs'][limb]}%)")
    
    def add_credits(self, amount):
        """Add credits to the player"""
        self.character_data["credits"] += amount
        messagebox.showinfo("Credits Added", f"You gained {amount} credits.")
        
        # Add note about credits gained
        self.add_note(f"Found {amount} credits. New balance: {self.character_data['credits']} credits.")
    
    def lose_credits(self, amount):
        """Subtract credits from the player (min 0)"""
        old_credits = self.character_data["credits"]
        self.character_data["credits"] = max(0, old_credits - amount)
        messagebox.showinfo("Credits Lost", f"You lost {amount} credits.")
        
        # Add note about credits lost
        self.add_note(f"Lost {amount} credits. New balance: {self.character_data['credits']} credits.")
    ...
    def show_load_game(self):
        ...
        if not save_files:
            ...
            for save_file in save_files:
                ...
                save_btn = tk.Button(saves_frame, text=player_name, font=("Arial", 14), width=20,
                                     command=lambda name=player_name: self.load_player(name)) # Use load_game_file with base name
        ...
        new_player_button = tk.Button(self.root, text="Create New Player", font=("Arial", 14), width=15, command=self.new_player_screen)
        new_player_button.pack(pady=20)
    ...
    def load_player(self, filename):
        self.player_data = Game.load_player_data(filename,self.base_path)
        self.story_mode_screen()

    def load_character(self, filename):
        try:
            self.player_data = Game.load_player_data(filename, self.base_path)
            self.character_data = self.player_data.get("Character")
            ...
            if "location" in self.character_data:
                x = self.character_data["location"].get("x", 0)
                y = self.character_data["location"].get("y", 0)
                ...
                if (x == 6 and y == 0) or (x == 0 and y == 6) or (x == 6 and y == 6) or (x == 6 and y == 3) or (x == 0 and y == -1) or (x == 3 and y == -1):
                    ...
                    self.update_player_data_from_room(self.character_data) # Use the callback logic to place player correctly
                elif x == -1 and y == 0:
    ...
    def add_burn_damage(self, min_damage, max_damage):
        ...
        # Apply damage
        original_damage = self.character_data["damage"].get("burn", 0)
        self.character_data["damage"]["burn"] = min(100, original_damage + damage)
        
        # Show damage message
        messagebox.showinfo("Burn Damage", f"You suffered {damage}% burn damage! Total burn damage: {self.character_data['damage']['burn']}%.")
        
        # Add note about the injury
        self.add_note(f"Suffered burn damage: Took {damage}% damage (total now {self.character_data['damage']['burn']}%)")
    ...
    def add_poison_damage(self, min_damage, max_damage):
        ...
        # Apply damage
        original_damage = self.character_data["damage"].get("poison", 0)
        self.character_data["damage"]["poison"] = min(100, original_damage + damage)
        
        # Show damage message
        messagebox.showinfo("Poison Damage", f"You suffered {damage}% poison damage! Total poison damage: {self.character_data['damage']['poison']}%.")
        
        # Add note about the injury
        self.add_note(f"Suffered poison damage: Took {damage}% damage (total now {self.character_data['damage']['poison']}%)")
    ...
    def enter_special_room(self, room_name):
        ...
        self.character_data["ship_map"] = self.ship_map
        
        room_instance = None
        if room_name == "MedBay":
            # Pass station_crew as a separate argument
            room_instance = MedBay(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Bridge":
            room_instance = Bridge(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Security":
            room_instance = Security(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Engineering":
            room_instance = Engineering(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Bar":
            room_instance = Bar(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Botany":
            room_instance = Botany(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
        elif room_name == "Quarters": # Quarters might not need crew data, adjust if needed
            room_instance = Quarters(self.root, self.character_data, self.update_player_data_from_room) # Assuming Quarters doesn't need crew

        # Clean up temporary data added to player_data after room is created
        if "ship_map" in self.character_data:
            del self.character_data["ship_map"]
    ...
    def enter_special_room_at(self, room_name, target_key):
        ...
        self.character_data["location"] = {"x": int(target_key.split(',')[0]), "y": int(target_key.split(',')[1])}
        ...
        if room_name == "MedBay":
            ...
            room_instance = Engineering(self.root, self.character_data, self.station_crew, self.update_player_data_from_room)
             ...
             room_instance = Quarters(self.root, self.character_data, self.update_player_data_from_room)
    ...
    def update_player_data_from_room(self, updated_player_data, updated_station_crew=None):
        ...
        # Update player data
        self.character_data = updated_player_data
        ...
        self.character_data["location"]["x"] = return_x
        self.character_data["location"]["y"] = return_y
    ...
    def add_random_item(self):
        ...
        self.character_data.setdefault("inventory", [])
            
        # Add the item dictionary to inventory
        self.character_data["inventory"].append(item_def)
    ...
    def add_market_knowledge(self):
        ...
        if not self.character_data["stock_market"]["companies"]:
            messagebox.showinfo("Market Tip", "You heard a stock tip, but don't understand the market yet.")
            return
            
        company = random.choice(self.character_data["stock_market"]["companies"])
    ...
    def drop_item(self, inventory_list, parent_popup):
        ...
        try:
            ...
            del self.character_data['inventory'][item_inventory_index]
    ...
    def drop_item_action(self, item_inventory_index, actions_popup, main_inventory_popup):
        ...
        if not (0 <= item_inventory_index < len(self.character_data['inventory'])):
             messagebox.showerror("Error", "Invalid item index provided for drop action.", parent=actions_popup)
             return

        item = self.character_data['inventory'][item_inventory_index]
